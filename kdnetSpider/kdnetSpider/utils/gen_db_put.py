...
from kdnetSpider.utils import MD5Utils
...
def gen_post_put(post_item):
    ...
    for key in post_item:
        if not key.startswith(('_', 'pictures')):
...
def gen_comment_put(comment_item):
    ...
    for key in comment_item:
        if not key.startswith(('_', 'pictures')):
            ...
            column_values.append(TColumnValue(picture_fml, (key + '_num').encode(), str(len(comment_item[key]))).encode())
...
def gen_fans_put(fans_item):
    ...
    fans_id = fans_item['fans']
    friends_id = fans_item['friends']
