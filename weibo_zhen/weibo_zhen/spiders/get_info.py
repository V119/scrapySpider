...
from weibo_zhen.items import UserInfoItem, BlogItem, CommentItem, TopicItem
from weibo_zhen.utils.page_info import get_page_conf_info, get_dom_html
...
def _get_comment_info(response, blog_id, html_=None, parent_comment_id='0'):
    ...
    if parent_comment_id == '0':
        ...
        try:
            ...
            comment_div = sel.xpath('//div[@class="list_box"]/div[@class="list_ul"]/div[@node-type="root_comment"]')
        except:
...
def get_comment_first_url(blog_id):
    ...
    :return:
    """
    url = 'http://weibo.com/aj/v6/comment/big?ajwvr=6&id=' + str(blog_id) + '&filter=all&from=singleWeiBo'
...
def get_comment_next_url(response):
    ...
    try:
        ...
        next_url = None
        if action_data:
            next_url = 'http://weibo.com/aj/v6/comment/big?ajwvr=6&' + action_data + '&from=singleWeiBo'
