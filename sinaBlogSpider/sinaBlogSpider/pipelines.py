...
from sinaBlogSpider.items import CommentItem, PostItem, AuthorItem
from sinaBlogSpider.utils.gen_db_put import gen_author_put, gen_post_put, gen_comment_put, gen_stop_spider_info, \
    gen_start_spider_info
...
class ImagePipeline(ImagesPipeline):
    ...
    def get_media_requests(self, item, info):
        if isinstance(item, PostItem) and 'picture_urls' in item and item['picture_urls']:
            for picture_url in item['picture_urls']:
                yield Request(picture_url)
        elif isinstance(item, AuthorItem) and 'picture_head_url' in item and item['picture_head_url']:
            for picture_url in item['picture_head_url']:
                yield Request(picture_url)
    ...
    def item_completed(self, results, item, info):
        ...
        if isinstance(item, PostItem):
            item['picture_local_path'] = local_uri
        elif isinstance(item, AuthorItem):
            item['picture_head_local'] = local_uri
